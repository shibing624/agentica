...
- [2026/08/10] **v1.4.12**: Context compression collapses to two layers — Layer 1 evicts old tool results under window pressure (~70%→50%), Layer 2 summarises with LLM/native compact; fixes the read-and-reread loop and Anthropic paths where eviction never ran. Layer 0 spills or truncates based on whether the session can read the path back; compaction counts land on `RunResponse.context_compactions`; CLI-only assumptions no longer dump on SDK callers (`enable_session_log=False`, no orphan `background` without a registry). See [Release-v1.4.12](https://github.com/shibing624/agentica/releases/tag/v1.4.12)
