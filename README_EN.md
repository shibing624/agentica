...
- [2026/08/11] **v1.4.13** (accumulating, not published): standing rules via plain `AGENTS.md` edits (no `remember` tool); user-level rules under `users/{user_id}/AGENTS.md` (CLI = `default`, with `~/.agentica/AGENTS.md` as a compatibility symlink); dropped `PERSONA.md`/`TOOLS.md`/`USER.md`; IM→CLI via `PEER_BRIDGE`; peer display distinguishes human vs agent; shell mode removed. See [CHANGELOG](https://github.com/shibing624/agentica/blob/main/CHANGELOG.md)
