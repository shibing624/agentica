...
class TestTheSwitchGatesDeciding:
    def test_with_the_switch_off_the_desktop_cannot_decide(self):
        """It is told (so it can show 'waiting'), but nothing waits on it."""
        desktop = _FakeDesktop(decision_body={"decision": "allow"})
        try:
            install_sink(NotifyConfig(enabled=True, socket=desktop.socket_path,
            ...
            async def scenario():
                ...
                waiter = registry.wait(pending)
                publish_approval(pending, registry, asyncio.get_running_loop())
                # Nothing should resolve it; the terminal prompt is the only path.
                with pytest.raises(asyncio.TimeoutError):
                    await asyncio.wait_for(waiter, timeout=0.8)

            _run_with_loop(scenario)
            time.sleep(0.2)
            # Only a notification went out, on /event, and no /await was made.
            assert all("/event" in r["request_line"] for r in desktop.requests)
        finally:
            desktop.close()
...
class TestTheLadderOnTheApprovalPath:
    def test_a_missing_desktop_app_does_not_hold_up_the_approval(self):
        ...
        install_sink(NotifyConfig(enabled=True, socket=_MissingDesktop().socket_path,
                                  approve_from_desktop=True, timeout_seconds=55))
