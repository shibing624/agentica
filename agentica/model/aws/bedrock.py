...
class AwsBedrock(Model):
    ...
    def get_aws_region(self) -> Optional[str]:
        ...
        from agentica.constants import AWS_REGION_ENV_VAR
    ...
    def get_aws_profile(self) -> Optional[str]:
        ...
        from os import getenv
        from agentica.constants import AWS_PROFILE_ENV_VAR
            ...
            if len(function_call_results) > 0:
                ...
                for _fc_message_index, _fc_message in enumerate(function_call_results):
                    tool_result = {
                        ...
                        "content": [{"json": json.dumps(_fc_message.content, ensure_ascii=False)}],
                    }
                    tool_result_message = {"role": "user", "content":
                        json.dumps([{"toolResult": tool_result}], ensure_ascii=False)}
                    fc_responses.append(tool_result_message)

                logger.debug(f"Tool call responses: {fc_responses}")
                messages.append(Message(role="user", content=json.dumps(fc_responses, ensure_ascii=False)))
