...
class Workflow(BaseModel):
    ...
    async def _prepare_run(self, args, kwargs):
    ...
    async def _process_result(self, result):
        ...
        if isinstance(result, (GeneratorType, collections.abc.Iterator)):
            ...
            return await self._wrap_single(result)
        else:
    ...
    def _wrap_generator(self, gen):
        ...
        def result_generator():
            for item in gen:
                ...
                yield item
            # Note: _finalize_run is async but generators can't await.
            # Use run_sync for the DB write inside generators.
            run_sync(self._finalize_run())

        return result_generator()

    async def _wrap_single(self, result: RunResponse):
    ...
    async def read_from_storage(self) -> Optional[WorkflowSession]:
        """Load from database."""
        if self.db is not None and self.session_id is not None:
            loop = asyncio.get_running_loop()
            session_row = await loop.run_in_executor(
                None, functools.partial(self.db.read_session, session_id=self.session_id, user_id=self.user_id)
            )
    ...
    async def write_to_storage(self) -> Optional[WorkflowSession]:
        """Save to database."""
        if self.db is not None:
            ...
            loop = asyncio.get_running_loop()
            await loop.run_in_executor(None, functools.partial(self.db.upsert_session, session_row))
            self._workflow_session = workflow_session
        return self._workflow_session

    async def load_session(self, force: bool = False) -> Optional[str]:
