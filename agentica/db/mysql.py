...
@description: MySQL implementation of BaseDb
"""
import json
import time
...
class MysqlDb(BaseDb):
    """
    MySQL implementation of unified database storage.
    ...
    def __init__(
        ...
        Args:
            ...
            db_url: SQLAlchemy database URL (overrides host/port/user/password/database)
        ...
        if _engine is None and db_url is not None:
            ...
            _db_url = f"mysql+pymysql://{user}:{password}@{host}:{port}/{database}?charset={charset}"
    ...
    def _get_memory_table(self) -> Table:
        if self._memory_table is None:
            self._memory_table = Table(
                ...
                Column("user_id", String(255), index=True),
                Column("memory", Text),  # JSON string
                Column("created_at", String(50)),  # ISO format
    ...
    def _get_knowledge_table(self) -> Table:
        if self._knowledge_table is None:
            ...
            self._knowledge_table = Table(
                ...
                Column("doc_type", String(50)),  # file type
                Column("size", BigInteger),  # file size
                Column("status", String(50)),  # processing status
    ...
    def __deepcopy__(self, memo):
        """Create a deep copy of the MysqlDb instance"""
        cls = self.__class__
