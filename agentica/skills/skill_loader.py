...
class SkillLoader:
    ...
    def get_search_paths(self, include_system: bool = False) -> List[tuple]:
        """
    ...
    def reload(
        ...
        registry.clear()
        return self.load_all(registry, include_system=include_system)
...
def load_skills(
    ...
    ``include_system`` defaults False: SDK callers must not pick up the
    CLI/gateway manuals (``agentica``, ``multi-agent``). Product entry
    points use :func:`load_system_skills` instead.
    ...
    return loader.load_all(include_system=include_system)
