# -*- coding: utf-8 -*-
"""
Markdown Converter for various file formats and web pages.
This is modified from Magentic-one's great repo:
...
class PlainTextConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        ...
        try:
            ...
            text_content = f"Failed to convert text file {local_path}: {e}"
...
class CsvConverter(DocumentConverter):
    """Anything with content type csv"""
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        ...
        content = DocumentConverter().convert(local_path).document.export_to_markdown()
        return DocumentConverterResult(
            title=local_path,
...
class JsonConverter(DocumentConverter):
    """Anything with content type json/jsonl"""

    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        extension = kwargs.get("file_extension", "")
        if extension.lower() not in [".json", ".jsonl"]:
            return None
...
class HtmlConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        # Bail if not html
        extension = kwargs.get("file_extension", "")
        if extension.lower() not in [".html", ".htm", ".xhtml"]:
            return None
        try:
            from docling.document_converter import DocumentConverter
        except (ImportError, ModuleNotFoundError):
...
class YouTubeConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        try:
            ...
            raise ImportError(
                "YouTubeConverter requires the youtube-transcript-api package. "
                "Please install it using `pip install youtube_transcript_api`."
            ) from e
        # Bail if not YouTube
        extension = kwargs.get("file_extension", "")
        if extension.lower() not in [".html", ".htm"]:
...
class PptxConverter(DocumentConverter):
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        # Bail if not a PPTX
        extension = kwargs.get("file_extension", "")
        if extension.lower() not in [".pptx", ".ppt"]:
            return None
        try:
            from docling.document_converter import DocumentConverter
        except (ImportError, ModuleNotFoundError):
...
class WavConverter(MediaConverter):
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        ...
        try:
            ...
            md_content += f"\n\n### Audio Transcript:\nError. Could not transcribe this audio. error: {e}"
    ...
    def _transcribe_audio(self, local_path) -> str:
        try:
            ...
            raise ImportError(
                "WavConverter requires the speech_recognition package. "
...
class Mp3Converter(WavConverter):
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        ...
        try:
            ...
            md_content += f"\n\n### Audio Transcript:\nError. Could not transcribe this audio: {str(e)}"
        finally:
            os.unlink(temp_path)
        return DocumentConverterResult(
            title=None,
...
class ZipConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        ...
        return DocumentConverterResult(
            title=f"{local_path}, Extracted Files",
            text_content=md_content.strip()
        )
...
class ImageConverter(MediaConverter):
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        # Bail if not a XLSX
        extension = kwargs.get("file_extension", "")
        if extension.lower() not in [".jpg", ".jpeg", ".png", ".tiff", ".bmp", ".webp"]:
            return None
        try:
            from docling.document_converter import DocumentConverter
        except (ImportError, ModuleNotFoundError):
...
class MarkdownConverter:
    ...
    def convert_local(self, path: str, **kwargs: Any) -> DocumentConverterResult:
        ...
        # Get extension alternatives from the path
...
if __name__ == '__main__':
    m = MarkdownConverter()
    r = m.convert("/Users/xuming/Documents/Codes/agentica/README.md")
    print(r)
    # Test with a local file
    result = m.convert("https://arxiv.org/pdf/2412.15166")
    print(f"Title: {result.title}")
    print(f"Content:\n{result.text_content}")

    r = m.convert("https://docling-project.github.io/docling/usage/supported_formats/")
    print(r)
