...
- **Subagents are now file-configured instead of enum-defined.** Packaged `explore` / `research` / `code` definitions live in `agentica/agents/*.md` with YAML frontmatter, while project and user files can override them from `.agentica/agents/` and `~/.agentica/agents/`. The default `review` subagent was removed so code review stays with the main agent and its full context. Partial runs still retain bounded tool inputs for one useful resume.
