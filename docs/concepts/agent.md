...
**DeepAgent 内置工具**（`ls`, `read_file`, `write_file`, `edit_file`, `apply_patch`, `glob`, `grep`, `execute`, `web_search`, `fetch_url`, `write_todos`, `task`, `ask_user_question`, `save_memory`, `search_memory`）

**DeepAgent 默认开启**：
- `enable_agentic_prompt=True`（Soul + Tools Guide + Heartbeat）
- 两层上下文压缩（淘汰 → 空窗换窗），旧轮次用 `search_session` / `read_session_item` 从 JSONL 取回，见 [Compression](../advanced/compression.md)
- Workspace 长期记忆
- Session Log 会话持久化
