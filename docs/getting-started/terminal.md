...
- **模型默认继承当前会话的 profile**（不是 `config.yaml` 里第一个同名 `model_name`）。会话里可用的就是**两个**：主模型和 auxiliary 模型（`--auxiliary_model_name`）；`model` 参数只能填这两个之一，填别的会被**拒绝**（拒绝信息列出可选的两个），不启动进程。这是 repo 级约定、不只 CLI：模型决定端点和账单，**换模型只有用户能做**，方式是换会话 profile（`/model <profile>`）；SDK 调用方在构造 agent 时就选定了 main/aux，同样不该被推翻。两个可用的模型都跑在**本会话 profile** 上（aux 以 `--model_name` 覆盖），所以不会串到父会话的 base_url——aux 若在别的端点，用 `--model_provider`/`--model_name` 那种写法会把它带歪。API key 不会出现在命令行上，子进程自己读 `config.yaml` / 环境变量。
