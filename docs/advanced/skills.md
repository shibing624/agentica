...
- 确认过的常驻偏好直接写进该用户的 `~/.agentica/workspace/users/{user_id}/AGENTS.md`（CLI 为 `.../users/default/AGENTS.md`，也可通过 symlink `~/.agentica/AGENTS.md` 改同一份文件；人手写或 agent `apply_patch`；不要再走 memory→AGENTS 编译）
