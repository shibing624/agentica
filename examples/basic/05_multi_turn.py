...
async def main():
    ...
    print(await agent_no_history.run("我叫什么名字?"))  # Won't remember
    ...
    print(await agent_with_history.run("我叫什么名字？做什么工作?"))
    ...
    response1 = await agent.run("我最喜欢的颜色是蓝色")
    print(f"Turn 1: {response1}")

    response2 = await agent.run("我喜欢吃川菜")
    print(f"Turn 2: {response2}")

    response3 = await agent.run("总结一下你了解到的关于我的信息")
